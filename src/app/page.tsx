    ...
    <main className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white text-slate-800">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-24 text-center">
          ...
          <p className="text-lg md:text-xl text-slate-500 max-w-3xl mx-auto mb-4 leading-relaxed">
            {t('hero.subheadline')}
          </p>
          <p className="text-sm text-slate-400 max-w-2xl mx-auto mb-12 leading-relaxed">
            {lang === 'en'
              ? 'Five detailed reports, written documents you keep forever.'
              : 'Cinco reportes detallados, documentos escritos que conservas para siempre.'}
          </p>
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-xl md:text-2xl text-slate-700 leading-relaxed mb-8">
            {lang === 'en'
              ? 'Most people have talked about themselves thousands of times. To therapists, to friends, to journals. But nobody has ever taken all of it and handed it back as a single coherent picture. That is what happens here. One conversation. Everything you share gets mapped. You leave holding a portrait of yourself that did not exist before you walked in.'
              : 'La mayoria de las personas ha hablado de si misma miles de veces. A terapeutas, a amigos, en diarios. Pero nadie ha tomado todo eso y lo ha devuelto como una sola imagen coherente. Eso es lo que sucede aqui. Una conversacion. Todo lo que compartes queda mapeado. Sales sosteniendo un retrato de ti mismo que no existia antes de entrar.'}
          </p>
          <p className="text-base text-slate-500 italic leading-relaxed">
            {lang === 'en'
              ? 'You already know yourself. You have just never seen yourself all at once.'
              : 'Ya te conoces. Simplemente nunca te has visto todo de una vez.'}
          </p>
        </div>
      </section>
      ...
      <section className="py-16 px-6 bg-slate-900">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-[10px] font-mono tracking-[0.35em] text-teal-400 uppercase mb-5">
            {lang === 'en' ? 'Try it before you commit' : 'Pruebalo antes de comprometerte'}
          </p>
          <h2 className="text-2xl md:text-3xl font-light text-white mb-4">
            {lang === 'en'
              ? 'Get a free map fragment. Five questions, five minutes.'
              : 'Obten un fragmento de mapa gratis. Cinco preguntas, cinco minutos.'}
          </h2>
          <p className="text-slate-400 leading-relaxed mb-8 max-w-lg mx-auto">
            {lang === 'en'
              ? 'Answer five questions in Dr. David\'s voice. Submit your email. Receive a short written portrait of what you shared, generated by the same AI methodology behind every full Terrain Map. No commitment, no sales follow-up. This is the same methodology behind every full Terrain Map, just a glimpse of it.'
              : 'Responde cinco preguntas. Recibe un retrato escrito breve de lo que compartiste, generado por la misma metodologia de IA detras de cada Terrain Map completo. Sin compromiso, sin seguimiento de ventas. Es la misma metodologia detras de cada Terrain Map completo, solo un vistazo.'}
          </p>
          <a
            href="/mini-map"
      ...
      <section className="py-24 px-6 bg-slate-900 text-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-[10px] font-mono tracking-[0.35em] text-teal-400 uppercase mb-4">The Response Map</p>
            <h2 className="text-2xl md:text-3xl font-medium text-white mb-4">
              When something difficult lands, people respond one of four ways.
            </h2>
            <p className="text-slate-400 text-base max-w-2xl mx-auto leading-relaxed">
              Most support systems are built around three of them. ReLoHu is built around the fourth.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-20">
            {[
              {
                category: 'Moving Toward',
                tag: 'Witnessing',
                highlight: true,
                items: [
                  { label: 'Full presence', desc: 'Attention without agenda' },
                  { label: 'Receiving what is given', desc: 'Not filtering it through a frame' },
                  { label: 'Reflecting back accurately', desc: 'Not improving, interpreting, or redirecting' },
                  { label: 'Staying with complexity', desc: 'Not resolving it prematurely' },
                  { label: 'Being changed by what you hear', desc: 'Letting it land' },
                ],
              },
              {
                category: 'Moving to Solutions',
                tag: 'Fixing',
                highlight: false,
                items: [
                  { label: 'Advice-giving', desc: 'Here is what you should do' },
                  { label: 'Reframing', desc: 'Have you thought about it this way instead?' },
                  { label: 'Silver lining', desc: 'But look at what you have' },
                  { label: 'Diagnosis', desc: 'I think the real issue is...' },
                  { label: 'Action planning', desc: "Let's make a strategy" },
                ],
              },
              {
                category: 'Moving Away',
                tag: 'Distancing',
                highlight: false,
                items: [
                  { label: 'Topic change', desc: 'A pivot to something easier' },
                  { label: 'Minimizing', desc: "It's not as bad as you think" },
                  { label: 'Premature reassurance', desc: "I'm sure it will work out" },
                  { label: 'Humor', desc: 'Deflecting contact through lightness' },
                ],
              },
              {
                category: 'Moving Against',
                tag: 'Correcting',
                highlight: false,
                items: [
                  { label: 'Correction', desc: "That's not how it actually happened" },
                  { label: 'Comparison', desc: 'Other people have it worse' },
                  { label: 'Judgment', desc: "You shouldn't feel that way" },
                  { label: 'One-upping', desc: 'Let me tell you about my experience' },
                ],
              },
            ].map((block, i) => (
              <div
                key={i}
                className={`rounded-2xl border p-6 ${block.highlight ? 'bg-teal-900/40 border-teal-700/50' : 'bg-slate-800/50 border-slate-700/50'}`}
              >
                <div className="flex items-center justify-between mb-5">
                  <h3 className={`text-sm font-semibold tracking-widest uppercase ${block.highlight ? 'text-teal-300' : 'text-slate-400'}`}>
                    {block.category}
                  </h3>
                  <span className={`text-[10px] font-mono tracking-widest px-2 py-0.5 rounded-full ${block.highlight ? 'bg-teal-800/60 text-teal-300' : 'bg-slate-700/60 text-slate-500'}`}>
                    {block.tag}
                  </span>
                </div>
                <ul className="space-y-3">
                  {block.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-3">
                      <div className={`w-1 h-1 rounded-full mt-2 flex-shrink-0 ${block.highlight ? 'bg-teal-400' : 'bg-slate-600'}`} />
                      <div>
                        <span className={`text-sm font-medium ${block.highlight ? 'text-teal-100' : 'text-slate-300'}`}>{item.label}</span>
                        <span className={`text-sm ml-1.5 ${block.highlight ? 'text-teal-300/70' : 'text-slate-500'}`}>-- {item.desc}</span>
                      </div>
                    </li>
                  ))}
                </ul>
          ...
          {/* Two-column contrast */}
          <div className="grid md:grid-cols-2 gap-0 rounded-2xl overflow-hidden border border-slate-700">
            <div className="bg-slate-800 p-8 border-b md:border-b-0 md:border-r border-slate-700">
              <p className="text-[10px] font-mono tracking-[0.35em] text-slate-500 uppercase mb-5">When you tell a therapist you&apos;re not doing well</p>
              <p className="text-slate-400 leading-relaxed">
                Your disclosure becomes material to work with. The therapist is listening toward something -- a pattern to address, a treatment goal. Even the most empathic clinician is simultaneously categorizing. What you share gets metabolized into a clinical frame. It becomes evidence of something that needs to change.
              </p>
            </div>
            <div className="bg-teal-900/30 p-8">
              <p className="text-[10px] font-mono tracking-[0.35em] text-teal-400 uppercase mb-5">When you tell ReLoHu you&apos;re not doing well</p>
              <p className="text-slate-300 leading-relaxed">
                Your disclosure becomes part of your map. It isn&apos;t evidence of a problem -- it&apos;s terrain. It gets witnessed and placed in relation to everything else you are. You&apos;re not a patient presenting symptoms. You&apos;re a landscape being seen clearly, possibly for the first time.
              </p>
            </div>
          </div>

          <p className="text-center text-slate-400 text-base mt-12 max-w-2xl mx-auto leading-relaxed">
            ReLoHu is built around the first category. Witnessing is not a byproduct of the work -- it is the work.
          </p>
        </div>
      </section>
            ...
            <div className="px-8 pt-8 pb-6 border-b border-slate-100">
              ...
              <div className="space-y-3">
                {(lang === 'en' ? [
                  ...
                  { icon: Layers,    text: 'A single-sitting synthesis: everything you shared, assembled into one coherent portrait you can hold, share, and return to.' },
                ] : [
                  { icon: Phone,     text: 'Llamada de orientacion de 15 min, gratis, sin compromiso' },
                  { icon: Clock,     text: 'Conversacion de intake de 90 minutos con Dr. David' },
                  { icon: FileText,  text: 'Cinco reportes detallados entregados como documentos' },
                  { icon: BookOpen,  text: 'Sesion de lectura del Terrain Map con Dr. David' },
                  { icon: Layers,    text: 'Una sintesis de una sola sesion: todo lo que compartiste, reunido en un retrato coherente que puedes conservar, compartir y revisitar.' },
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            {/* Placeholder for Evelyn's testimonial */}
            <Card className="bg-white border-dashed border-slate-200 shadow-none">
              <CardContent className="p-6 flex flex-col items-center justify-center min-h-[200px] text-center">
                <div className="flex items-center gap-1 mb-4">
                  {[...Array(5)].map((_, s) => (
                    <svg key={s} className="w-4 h-4 text-slate-200 fill-slate-200" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                  ))}
                </div>
                <p className="text-slate-300 text-sm italic mb-6">Testimonial pending</p>
                <div className="pt-4 border-t border-slate-100 w-full">
                  <p className="text-xs text-slate-300 italic">Therapist, Florida</p>
                </div>
              </CardContent>
            </Card>
          </div>
